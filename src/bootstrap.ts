...
/**
 * Everything the application is, minus listening on a port.
 *
 * SEPARATE FROM `main.ts` SO TESTS GET THE REAL APPLICATION. A test that
 * assembles the modules itself is testing a different program: no cookie
 * parser, no validation pipe, no global prefix, no versioning. It would then
 * pass while the running API was broken — and the one HTTP contract nobody had
 * exercised end to end, `POST /auth/refresh`, was returning a token without an
 * identity, which every unit test was happy with and the browser was not.
 *
 * The caller creates the app, because two options can only be set at creation:
 * `bodyParser: false` (see below) and `bufferLogs`.
 */
export function configureApp(app: NestExpressApplication): void {
  ...
  //
  //    Only meaningful because the app was created with `bodyParser: false`:
  //    otherwise the adapter has already registered its own parser at 100 KB,
  //    the first parser to see a request wins, and these two never run.
